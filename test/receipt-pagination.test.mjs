...
for (const count of [0, 1, 999, 1000, 1001, 1100, 2501]) {
  test(`absence requires a terminal page after ${count} comments`, async (t) => {
    const calls = mockPages(t, p => pageOf(p, Math.max(0, Math.min(100, count - (p - 1) * 100))));
    assert.equal(await scan(), null);
    assert.deepEqual(calls, Array.from({ length: Math.floor(count / 100) + 1 }, (_, i) => i + 1));
  });
}
...
// Exercise the actual runner: new requests proceed only after complete lookup;
// historic duplicates/conflicts and API failures never reach target execution.
for (const mode of ['new', 'duplicate', 'conflict', 'api-error']) {
...
const calls=[];process.on('exit',()=>fs.writeFileSync(${JSON.stringify(callsPath)},JSON.stringify(calls)));
...
if(method==='GET'){const p=Number(u.searchParams.get('page'));if(p<=10)return new Response(JSON.stringify(Array.from({length:100},(_,i)=>({id:(p-1)*100+i+1,body:'ordinary'}))));
if(${JSON.stringify(mode)}==='api-error')return new Response('{}',{status:403});
...
return new Response(JSON.stringify({id:999999,body:body.body}));};`);
    ...
    assert.equal(calls.filter(c => c.method === 'GET').length, 11);
    ...
    if (mode === 'new') {
      ...
      assert.match(child.stderr, mode === 'conflict' ? /REQUEST_ID_CONFLICT/ : /GITHUB_API_ERROR/);
